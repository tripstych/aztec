...
function jupiterx_enqueue_jupiterx_default() {
	if (
		function_exists( 'jupiterx_compile_less_fragments' ) &&
		! jupiterx_check_default()
	) {
		return;
	}
...
function jupiterx_enqueue_jupiterx_components() {
	...
	$frontend_js = jupiterx_check_default() ? 'frontend-light' : 'frontend';

	// Scripts.
	wp_enqueue_script(
		'jupiterx',
		JUPITERX_ASSETS_URL . 'dist/js/' . $frontend_js . JUPITERX_MIN_JS . '.js',
...
function jupiterx_get_theme_assets() {
	...
	$assets['styles'] = [
		...
		'theme/animations',
		'theme/site',
		'theme/header', // if we want to remove header default html structure.
		'theme/main',
		'theme/post',
		'theme/post-single',
...
function jupiterx_get_wc_assets() {
	...
	if ( ! class_exists( 'woocommerce' ) || jupiterx_check_default() ) {
		return $assets;
	}
...
function jupiterx_editor_styles() {
	$disable_editor_styles = apply_filters( 'jupiterx_disable_editor_styles', false );

	if ( ! function_exists( 'jupiterx_compile_less_fragments' ) || $disable_editor_styles ) {
		return;
	}
