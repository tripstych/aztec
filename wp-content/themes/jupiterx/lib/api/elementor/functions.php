...
add_filter( 'template_include', 'jupiterx_maintenance_template_handler', 99999, 1 );

if ( ! function_exists( 'jupiterx_maintenance_template_handler' ) ) {
	/**
	 * Handle maintenace template.
	 *
	 * @since 4.2.0
	 * @param string $template Template path.
	 * @return string
	 */
	function jupiterx_maintenance_template_handler( $template ) {
		if ( ! class_exists( '\Elementor\Plugin' ) ) {
			return $template;
		}
		...
		if ( ! empty( $elementor_maintenance_mode ) && jupiterx_check_default() ) {
			$check_maintenace_template = jupiterx_elementor_maintenance_template();

			if ( ! $check_maintenace_template ) {
				return $template;
			}
		...
		if ( ! jupiterx_check_default() && ! empty( $is_enabled ) ) {
			return $template;
		}

		if ( ! jupiterx_check_default() && empty( $is_enabled ) && ! empty( $elementor_maintenance_mode ) ) {
			$check_maintenace_template = jupiterx_elementor_maintenance_template();
...
if ( ! function_exists( 'jupiterx_elementor_maintenance_template' ) ) {
	/**
	 * Making jupiterx templates compatible with elementor maintenance mode.
	 *
	 * @since 4.2.0
	 * @return boolean
	 */
	function jupiterx_elementor_maintenance_template() {
		...
		$elementor_maintenance_page_id = intval( get_option( 'elementor_maintenance_mode_template_id', 0 ) );

		if ( $elementor_maintenance_page_id !== $page_id ) {
			return false;
		}
