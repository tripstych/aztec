...
if ( ! function_exists( 'jupiterx_get_plugins_from_api' ) ) {
	...
	function jupiterx_get_plugins_from_api() {
		$response = wp_remote_get( 'https://my.artbees.net/wp-json/plugins/v1/list?theme_name=jupiterx&order=ASC&orderby=menu_order' );
		...
		foreach ( $plugins as $key => $plugin ) {
			if ( jupiterx_check_default() && 'advanced-custom-fields' === $plugin['slug'] ) {
				unset( $plugins[ $key ] );
				continue;
			}
		...
		$slug_keys_list = array_column( $plugins_list, 'slug' );

		if ( ! empty( $repo_plugins ) && ! empty( $slug_keys_list ) ) {
			$repo_plugins = jupiterx_get_wp_plugins_info( $slug_keys_list );
			...
			foreach ( $repo_plugins as $slug => $info ) {
				...
				$plugins_list[ $slug ]['version'] = null;

				if ( array_column( $info, 'version' ) ) {
					$plugins_list[ $slug ]['version'] = $info['version'];
				}
				
				$plugins_list[ $slug ]['desc'] = null;

				if ( array_column( $info, 'short_description' ) ) {
					$plugins_list[ $slug ]['desc'] = $info['short_description'];
				}

				$plugins_list[ $slug ]['img_url'] = null;

				if ( array_column( $info, 'icons' ) ) {
...
if ( ! function_exists( 'jupiterx_has_required_plugins_activated' ) ) {
	...
	function jupiterx_has_required_plugins_activated() {
		...
		if ( ! method_exists( $tgmpa, 'is_the_plugin_active' ) ) {
			return false;
		}
		...
		foreach ( $plugins as $plugin ) {
			if ( isset( $tgmpa->plugins[ $plugin['slug'] ] ) && ! $tgmpa->is_the_plugin_active( $plugin->slug ) ) {
				return false;
			}
		}
