...
function jupiterx_get_layout() {
	...
	$layout = apply_filters( 'jupiterx_layout', $layout );

	if ( jupiterx_check_default() ) {
		$layout = get_taxonomy_simplicity_layout( 'c' );
	}
...
/**
 * Get taxonomy layout on simplicity mode.
 *
 * @param string $layout Current layout.
 * @since 3.8.0
 * @return string
 */
function get_taxonomy_simplicity_layout( $layout ) {
	if ( is_category() || is_tag() || is_tax() ) {
		$term_layout = '';

		$values = get_queried_object();

		if ( is_object( $values ) && ! empty( $values ) ) {
			$term_layout = get_term_meta( $values->term_id, 'jupiterx_layout', true );
		}
