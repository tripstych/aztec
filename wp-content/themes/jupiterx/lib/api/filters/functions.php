<?php
/**
 * Jupiter Filters extend WordPress Filters by registering sub filters if they are told to do so.
 *
 * @package JupiterX\Framework\API\Filters
 */
 ...
 *                                          Jupiter creates an anonymous function to hook into the filter event.
 ...
 */
function jupiterx_add_filter( $hook, $callback_or_value, $priority = 10, $args = 1 ) {

	if ( is_callable( $callback_or_value ) ) {
		return add_filter( $hook, $callback_or_value, $priority, $args );
	}

	return _jupiterx_add_anonymous_filter( $hook, $callback_or_value, $priority, $args );
}
 ...
 */
function jupiterx_apply_filters( $id, $value ) {
	$args = func_get_args();
 ...
 *                  value.
 */
function jupiterx_has_filters( $id, $callback = false ) {
 ...
 *
 * @return _JupiterX_Anonymous_Filters
 */
function _jupiterx_add_anonymous_filter( $hook, $value, $priority = 10, $args = 1 ) {
	require_once JUPITERX_API_PATH . 'filters/class-anonymous-filters.php';

	return new _JupiterX_Anonymous_Filters( $hook, $value, $priority, $args );
}
