...
var ProductFilter = elementorModules.frontend.handlers.Base.extend({
  getDefaultSettings: function getDefaultSettings() {
    return {
      selectors: {
        ...
        ProductFilterBrandToggle: '.sellkit-brand-filter-toggle .product-filter-item',
  ...
  getDefaultElements: function getDefaultElements() {
    ...
    return {
      ...
      $productFilterBrandToggle: this.$element.find(selectors.ProductFilterBrandToggle),
  ...
  handleBrandToggle: function handleBrandToggle() {
    this.elements.$productFilterBrandToggle.on('click', function (event) {
      var filters = $(event.currentTarget).parent().find('.product-filter-item');
      $(event.currentTarget).toggleClass('sellkit-brand-toggle');
  ...
  productsSettings: function productsSettings(wrapper) {
    var productSettings = wrapper.data('settings'),
        ...
        atcButton = productWrapperSettings.atc_button,
        productsData = [];
    productsData.push(productsColumn, productsRow, productsPagination, productsPaginationType, productsAllProduct, productLayout, productSwapEffect, productsImageSize, productsAtcButton, productsAtcButtonIcon, productsWishlist, productsNumber, productGeneralLayout, productContentLayout, atcButton);
    return productsData;
  },
  ...
  handleAjax: function handleAjax(parameter) {
    ...
    wp.ajax.send('sellkit_get_products', {
      ...
      success: function success(data) {
        ...
        _this19.handleScroll(parameter);
  ...
  handleScroll: function handleScroll() {
    var _element$, _element$$classList;

    var element = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : null;

    if ('yes' !== this.getElementSettings('scroll_to_top')) {
      return;
    }

    if (element !== null && ((_element$ = element[0]) === null || _element$ === void 0 ? void 0 : (_element$$classList = _element$.classList) === null || _element$$classList === void 0 ? void 0 : _element$$classList.contains('sellkit-product-filter-clear'))) {
      return;
    }

    $('html, body').animate({
      scrollTop: $('.elementor-widget-sellkit-product-filter').offset().top - 50
    }, 500);
  }
});
