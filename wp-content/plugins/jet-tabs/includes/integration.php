...
if ( ! class_exists( 'Jet_Tabs_Integration' ) ) {
	...
	class Jet_Tabs_Integration {
		...
		public function init() {
			...
			add_action( 'save_post', [ $this, 'save_elementor_template_post_type'], 10, 3 );
		 ...
		 * @return void
		 */
		public function save_elementor_template_post_type( $template_id, $post, $update ) {

			if ( empty( $post ) || 'elementor_library' !== $post->post_type  ) {
				return;
			}

			\Jet_Cache\Manager::get_instance()->db_manager->delete_cache_by_instance_id( $template_id, 'elementor_library' );
		}
