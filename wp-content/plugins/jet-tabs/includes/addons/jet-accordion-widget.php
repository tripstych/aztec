...
use Elementor\Utils;
use Jet_Tabs\JSON_LD_Schema;

if ( ! defined( 'ABSPATH' ) ) exit; // Exit if accessed directly

if ( ! class_exists( 'JSON_LD_Schema' ) ) {
	require_once plugin_dir_path( dirname( __FILE__ ) ) . 'base/json-ld-schema.php';
}
		...
		<div <?php echo $this->get_render_attribute_string( 'instance' ); ?>>
			<div class="jet-accordion__inner">
				<?php
					...
					do_action( 'jet-engine-query-gateway/before-loop', 'toggles', $this );
					...
					foreach ( $toggles as $index => $item ) {
						...
						?><div <?php echo $this->get_render_attribute_string( $toggle_setting_key ); // phpcs:ignore ?>>
							<div <?php echo $this->get_render_attribute_string( $toggle_control_setting_key ); // phpcs:ignore ?>>
								<?php echo $toggle_control_icon_html; // phpcs:ignore
									echo $toggle_control_label_html; // phpcs:ignore ?>
							</div>
							<div <?php
							echo $this->get_render_attribute_string( $toggle_content_setting_key ); // phpcs:ignore ?>>
								<div class="jet-toggle__content-inner"><?php echo $content_html; // phpcs:ignore ?></div>
							</div>
						</div><?php
						if ( 'yes' === $faq_schema ) {
							JSON_LD_Schema::instance()->add_item( $item['item_label'], $content_html );
						}
					}
