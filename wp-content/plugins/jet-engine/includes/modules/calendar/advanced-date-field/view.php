...
class Jet_Engine_Advanced_Date_Field_View {
	...
	public function prepare_field_args( $args, $field, $instance ) {
		...
		$args['extra_attr'] = [
			'data-allow-time' => ! empty( $field['allow_timepicker'] ) ? filter_var( $field['allow_timepicker'], FILTER_VALIDATE_BOOLEAN ) : false,
			'data-format' => ! empty( $field['field_ui_format'] ) ? $field['field_ui_format'] : 'rrule',
		];
	...
	public function enqueue_assets( $hook = false ) {
		...
		add_action( 'admin_print_footer_scripts', array( $this, 'print_field_js_template_rrule' ) );
		add_action( 'admin_print_footer_scripts', array( $this, 'print_field_js_template_custom' ) );
	...
	public function print_field_js_template_custom() {
		?>
		<script type="text/html" id="tmpl-jet-engine-advanced-date-field-custom">
		<?php $this->print_field_js_template_base_date(); ?>
		</script>
		<?php
	}
