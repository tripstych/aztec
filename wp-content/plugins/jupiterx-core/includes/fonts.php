...
class JupiterX_Custom_Fonts {
	...
	public function get_all_font_faces() {
		...
		foreach ( $all_font_faces as $font_face ) {
			$font_faces .= PHP_EOL . htmlspecialchars( $font_face->font_face, ENT_NOQUOTES ) . PHP_EOL;
		}

		echo '<style id="jupiterx-editor-custom-font-faces">' . $font_faces . '</style>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	}
}
