 ...
 */
class JupiterX_Core_Control_Panel_WXR_Importer extends WP_Importer {
	/**
	 ...
	 * Logger instance.
	 *
	 * @var JupiterX_Core_Control_Panel_Importer_Logger
	 */
	protected $logger;
	...
	public function get_preliminary_information( $file ) {
		...
		// Start parsing!
		$data = new JupiterX_Core_Control_Panel_WXR_Import_Info();
		while ( $reader->read() ) {
	...
	protected function parse_post_node( $node ) {
		...
		foreach ( $node->childNodes as $child ) {
			...
			switch ( $child->tagName ) {
				...
				case 'wp:comment':
					$comment_enabled = apply_filters( 'jupiterx_import_template_enable_comments', false );
	...
	protected function process_post( $data, $meta, $comments, $terms ) {
		...
		if ( 'attachment' === $postdata['post_type'] ) {
			if ( ! $this->options['fetch_attachments'] && ! $this->jupiterx_is_elementor_svg_icon_attachment( $meta ) ) {
	...
	protected function process_attachment( $post, $meta, $remote_url ) {
		...
		$this->jupiterx_override_elementor_svg_icon_attachment( $upload, $meta );
	...
	protected function process_post_meta( $meta, $post_id, $post ) {
		...
		foreach ( $meta as $meta_item ) {
			...
			if ( $this->elementor_inline_svg_meta === $meta_item['key'] && ! $this->options['fetch_attachments'] ) {
				$key = $this->elementor_inline_svg_meta;
			}
	...
	protected function process_term( $data, $meta ) {
		...
		if ( $this->is_new_woocommerce_attribute( $data['taxonomy'] ) ) {
			wc_create_attribute( [
				'name' => str_replace( 'pa_', '', $data['taxonomy'] ),
			] );

			register_taxonomy( $data['taxonomy'], 'product' );
		}
	...
	/**
	 * Check attachment is Elementor svg icon.
	 *
	 * @since 1.16.1
	 * @access protected
	 *
	 ...
	 * @return boolean
	 */
	protected function jupiterx_is_elementor_svg_icon_attachment( &$meta ) {
		...
		foreach( $meta as &$meta_item ) {
			...
			if ( $this->elementor_inline_svg_meta === $meta_item['key'] ) {
				$meta_item['value'] = $this->elementor_inline_svg_icon;
	...
	/**
	 * Override Elementor Icon element svg with dummy Jupiter X logo.
	 *
	 * @param array $upload Upload file.
	 * @param array $meta Attachment meta.
	 * @return void
	 */
	protected function jupiterx_override_elementor_svg_icon_attachment( $upload, $meta ) {
		if (
			empty( $upload['file'] ) ||
			$this->options['fetch_attachments'] ||
			! $this->jupiterx_is_elementor_svg_icon_attachment( $meta )
		) {
			return;
		}
		...
		$jupiterx_filesystem = new JupiterX_Core_Control_Panel_Filesystem(
			[
				'context' => $wp_upload_dir['basedir'],
			]
		);

		if ( ! $jupiterx_filesystem->exists( $upload['file'] ) ) {
			return;
		}

		$jupiterx_filesystem->put_contents( $upload['file'], $this->elementor_inline_svg_icon );
	}

	/**
	 * Checks if it's a new woocommerce attribute or not.
	 *
	 * @param $taxonomy
	 * @return bool
	 */
	protected function is_new_woocommerce_attribute( $taxonomy ) {
		...
		$attributes_names = wp_list_pluck( $attributes, 'attribute_name' );

		if ( in_array( $taxonomy, array_values( $attributes_names ) ) ) {
			return false;
		}
