...
class JupiterX_Core_Condition_Manager {
	...
	*/
	public function __construct() {
		...
		add_action( 'wp_trash_post', [ $this, 'remove_page_title_bar_transient' ], 10, 1 );
	}

	/**
	 * Remove page title bar transient.
	 *
	 * @param int $post_id Post ID.
	 * @since 4.4.0
	 */
	public function remove_page_title_bar_transient( $post_id ) {
		$template_type = get_post_meta( $post_id, '_elementor_template_type', true );

		if ( 'page-title-bar' !== $template_type ) {
			return;
		}
	...
	public function register_documents( $documents_manager ) {
		...
		if ( ! defined( 'ELEMENTOR_PRO_VERSION' ) && class_exists( 'WooCommerce' ) ) {
