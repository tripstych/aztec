var kirkiDependencies = {
	...
	showKirkiControl: function( control ) {
		...
		if ( _.isArray( control.params.required ) ) {
			for ( i = 0; i < control.params.required.length; i++ ) {
				if ( ! self.checkCondition( control.params.required[ i ], control, isOption, relation ) ) {
					show = false;
				}
			}
		} else if ( _.isObject( control.params.required ) ) {
			// console.log( control.params.required );

			relation = _.isUndefined( control.params.required.relation ) ? relation : control.params.required.relation;

			for ( var term in control.params.required.terms ) {
				nestedItems.push( self.checkCondition( control.params.required.terms[ term ], control, isOption, relation ) );
			}

			if ( 'OR' === relation ) {
				show = ( -1 !== nestedItems.indexOf( true ) );
			}

			if ( 'AND' === relation ) {
				show = ( -1 === nestedItems.indexOf( false ) );
			}
		}
	...
	checkCondition: function( requirement, control, isOption, relation ) {
		...
		if ( ( _.isArray( requirement ) || _.isObject( requirement ) ) && 'undefined' === typeof requirement.setting ) {
			nestedItems = [];

			// Loop sub-requirements.
			if ( _.isArray( requirement ) ) {
				childRelation = ( 'AND' === childRelation ) ? 'OR' : 'AND';

				for ( i = 0; i < requirement.length; i++ ) {
					nestedItems.push( self.checkCondition( requirement[ i ], control, isOption, childRelation ) );
				}
			} else if ( _.isObject( requirement ) ) {
				childRelation = _.isUndefined( requirement.relation ) ? 'AND' : requirement.relation;

				for ( var term in requirement.terms ) {
					nestedItems.push( self.checkCondition( requirement.terms[ term ], control, isOption, childRelation ) );
				}
			}
