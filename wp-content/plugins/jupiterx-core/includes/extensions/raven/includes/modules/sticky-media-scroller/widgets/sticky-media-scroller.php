...
class Sticky_Media_Scroller extends Base_Widget {
		...
		<div <?php echo $this->get_render_attribute_string( 'widget-wrapper' ); ?>>
			<div class="sticky-mode-wrapper">
				...
				<div class="content-column">
					<?php echo $this->render_column( $settings, 'content' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
				</div>
			</div>
			<div class="responsive-mode-wrapper">
				<?php echo $this->render_responsive_content_and_media(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
			</div>
		</div>
		<?php
	...
	private function render_column( $settings, $column ) {
		...
		foreach ( $settings['sections'] as $index => $section ) {
			...
			<div <?php echo $this->get_render_attribute_string( $column . '-section-wrapper' . esc_attr( $section['_id'] ) ); ?>>
				...
				if ( 'content' === $column ) {
					echo $this->render_section_content( $section ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				}
				?>
			</div>
			<?php
	...
	private function render_image( $section ) {
		if ( empty( $section['image']['id'] ) ) {
			...
			// WPML compatibility.
			$section['image']['id'] = apply_filters( 'wpml_object_id', $section['image']['id'], 'attachment', true );
			$attachment_url         = wp_get_attachment_image_src( $section['image']['id'], 'full' );

			if ( $attachment_url ) {
				$section['image']['url'] = $attachment_url[0];
			}

			$alt = get_post_meta( $section['image']['id'], '_wp_attachment_image_alt', true );

			if ( ! empty( $alt_text ) ) {
				$section['image']['alt'] = $alt;
			}
