...
class Render_Helper {
	...
	public function show_heading( $settings ) {
		...
		<?php if ( ! empty( $settings['heading'] ) ) : ?>
			<?php echo '<' . $heading_tag . ' ' . $heading_attr . '>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
	...
	public function show_price( $settings ) {
		...
		$period_element  = '<span ' . $this->widget->get_render_attribute_string( 'period' ) . '>' . wp_kses_post( $price_period ) . '</span>';
		?>
		<div class="raven-pricing-table__price">
			...
			if ( $price_period && 'below' === $period_position ) :
				Utils::print_unescaped_internal_string( $period_element );
			endif;
			?>
		</div>
		<?php
