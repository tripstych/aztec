...
abstract class Skin_Base extends Elementor_Skin_Base {
	...
	protected function render_lightbox() {
		...
		<div class="raven-photo-album-lightbox">
			<?php foreach ( $images as $image ) : ?>
				...
				?>
				<a href="<?php echo esc_url( Group_Control_Image_Size::get_attachment_image_src( $image['id'], 'image', [ 'image_size' => 'full' ] ) ); ?>" data-elementor-lightbox-slideshow="<?php echo esc_attr( $this->parent->get_id() . $this->item['_id'] ); ?>"></a>
			<?php endforeach; ?>
		</div>
		<?php
	}
	...
	protected function render_title() {
		...
		printf( '<h3 class="raven-photo-album-title">%s</h3>', wp_kses_post( $this->item['title'] ) );
	}
	...
	protected function render_description() {
		...
		printf( '<p class="raven-photo-album-description">%s</p>', wp_kses_post( $this->item['description'] ) );
	}
