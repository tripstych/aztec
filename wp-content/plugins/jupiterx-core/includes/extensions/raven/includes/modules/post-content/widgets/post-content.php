...
class Post_Content extends Base_Widget {
	...
	public function show_in_panel() {
		$enable_post_content = apply_filters( 'jupiterx_raven_enable_post_content_widget', false );

		if ( $enable_post_content ) {
			return true;
		}
	...
	public function render() {
		...
		// Avoid recursion
		if ( ! isset( $post->ID ) || isset( $did_posts[ $post->ID ] ) ) {
			return;
		}
		...
		$with_wrapper = apply_filters( 'jupiterx_post_content_widget_wrapper', false );

		if ( $with_wrapper ) {
			echo '<div class="elementor-post__content jupiterx-post-content-wrapper">' . balanceTags( $content, true ) . '</div>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			return;
		}
