...
class Advanced_Nav_Menu extends Base_Widget {
	...
	protected function render() {
		...
		<nav <?php echo $this->get_render_attribute_string( 'menu' ); ?>>
			...
			if ( 'offcanvas' === $main_layout ) {
				...
				Elementor_Utils::print_unescaped_internal_string( $this->render_logo( 'side' ) );
			}

			Elementor_Utils::print_unescaped_internal_string( $menu_template );
			?>
		</nav>
		<?php
		...
		<nav <?php echo $this->get_render_attribute_string( 'mobile_menu' ); ?>>
			...
			<div class="raven-container">
				<?php Elementor_Utils::print_unescaped_internal_string( str_replace( "menu-{$id}", "menu-mobile-{$id}", $menu_template ) ); ?>
			</div>
		</nav>
		<?php
	}
	...
	private function render_toggle_button( $settings ) {
		...
		<div class="raven-adnav-menu-toggle">
			...
			if ( $is_hamburger ) {
						?>
						<div class="hamburger hamburger--<?php echo esc_attr( $animation ); ?>">
			...
			?>
				...
				?>
					<?php Elementor_Utils::print_unescaped_internal_string( $this->render_icon( $custom_icon ) ); ?>
				</div>
			</div>
		</div>
		<?php
	...
	public function render_logo( $type ) {
		$settings    = $this->get_settings_for_display();
		$logo_source = $settings[ "{$type}_logo_source" ];
		...
		foreach ( $devices as $device ) {
			...
			if ( in_array( $logo_skin, $logo_skins, true ) && 'custom-source' !== $logo_source ) {
			...
			if ( empty( $image_src ) || 'custom-source' === $logo_source ) {
				$image_src = Elementor_Utils::get_placeholder_image_src();

				if ( ! empty( $settings[ "{$type}_image{$device}" ]['url'] ) ) {
					$image_src = $settings[ "{$type}_image{$device}" ]['url'];

					$this->add_render_attribute( "{$type}_image{$device}", [
	...
	public function render_icon( $icon_data, $attrs = [] ) {
		if ( 'svg' === $icon_data['library'] ) {
			Elementor_Utils::print_unescaped_internal_string( Icon_Manager::render_uploaded_svg_icon( $icon_data['value'] ) );
			return;
		}

		Elementor_Utils::print_unescaped_internal_string( Icon_Manager::render_font_icon( $icon_data, $attrs ) );
	}
