...
class Hotspot extends Base_Widget {
	...
	protected function register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'type' => 'color',
				'default' => RavenUtils::set_old_default_value( '#6EC1E4' ),
	...
	protected function render() {
		...
		foreach ( $settings['hotspot'] as $key => $hotspot ) :
			...
			echo sprintf( '<%1$s %2$s>', Utils::validate_html_tag( $hotspot_element_tag ), $this->get_render_attribute_string( $hotspot_repeater_setting_key ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			?>
			...
			echo sprintf( '</%s>', Utils::validate_html_tag( $hotspot_element_tag ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		endforeach;
	}
