...
use JupiterX_Core\Raven\Modules\Heading\Module;
...
class Heading extends Base_Widget {
			...
			[
				...
				'fields_options' => [
					'background' => [
						...
						'default' => Module::get_default_value(),
					],
				],
	...
	protected function render() {
		...
		if ( array_key_exists( 'raven_animated_gradient_enable', $settings ) && 'yes' === $settings['raven_animated_gradient_enable'] ) {
			...
			if ( isset( $settings['title_color_background'] ) && 'gradient' === $settings['title_color_background'] ) {
				$element = 'title-inner';
			}
		...
		?>
		<div class="raven-widget-wrapper"><?php ElementorUtils::print_unescaped_internal_string( $heading_html ); ?></div>
		<?php
	}
