...
abstract class Skin_Base extends Elementor_Skin_Base {
	...
	public function register_controls( Widget_Base $widget ) {
		...
		$this->register_categories_count_controls();
	...
	protected function register_settings_controls() {
		...
		$this->add_control(
			'show_category_count',
			[
				'label' => esc_html__( 'Category Count', 'jupiterx-core' ),
	...
	protected function register_categories_count_controls() {
		$this->start_controls_section(
			'section_categories_count',
			[
				'label' => esc_html__( 'Category Count', 'jupiterx-core' ),
				'tab' => 'style',
			]
		...
		$this->add_control(
			'section_categories_count_border',
			[
				...
				'default'   => '',
				'selectors' => [
					'{{WRAPPER}} .raven-categories-count' => 'border-style: {{VALUE}};',
				],
			]
		);

		$this->add_responsive_control(
			'section_categories_count_border_radius',
			[
				'label'      => esc_html__( 'Border Radius', 'jupiterx-core' ),
				'type'       => Controls_Manager::DIMENSIONS,
				'size_units' => [ 'px', 'em', '%' ],
				'selectors'  => [
					'{{WRAPPER}} .raven-categories-count' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
				'default' => [
					'top' => 50,
					'right' => 50,
		...
		$this->add_control(
			'section_categories_count_border_width',
			[
				...
				'selectors' => [
					'{{WRAPPER}} .raven-categories-count' => 'border-width: {{SIZE}}{{UNIT}};',
				],
				'condition' => [
					$this->get_control_id( 'section_categories_count_border' ) => [ 'solid', 'double', 'dotted', 'dashed', 'groove' ],
				],
			]
		);

		$this->add_responsive_control(
			'section_categories_count_padding',
			[
				'label'      => esc_html__( 'Padding', 'jupiterx-core' ),
				'type'       => Controls_Manager::DIMENSIONS,
				'size_units' => [ 'px', 'em', '%' ],
				'selectors'  => [
					'{{WRAPPER}} .raven-categories-count' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);

		$this->add_responsive_control(
			'section_categories_count_margin',
			[
				'label'      => esc_html__( 'Spacing', 'jupiterx-core' ),
				'type'       => Controls_Manager::DIMENSIONS,
				'size_units' => [ 'px', 'em', '%' ],
				'selectors'  => [
					'{{WRAPPER}} .raven-categories-count' => 'margin: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
				'default' => [
					'left'     => '20',
					'unit'     => 'px',
		...
		$this->add_control(
			'section_categories_count_color',
			[
				'label'     => esc_html__( 'Text Color', 'jupiterx-core' ),
				'type'      => Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} .raven-categories-count' => 'color: {{VALUE}};',
				],
				'default' => '#FFFFFF',
			]
		);
		...
		$this->add_control(
			'section_categories_count_border_color',
			[
				'label'     => esc_html__( 'Border Color', 'jupiterx-core' ),
				'type'      => Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} .raven-categories-count' => 'border-color: {{VALUE}};',
				],
			]
		);

		$this->add_control(
			'section_categories_count_background_color',
			[
				'label'     => esc_html__( 'Background Color', 'jupiterx-core' ),
				'type'      => Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} .raven-categories-count' => 'background-color: {{VALUE}};',
				],
				'default' => '#BFBFBF',
			]
		);
		...
		$this->add_group_control(
			...
			[
				'name' => 'section_categories_count_typography',
				'selector' => '{{WRAPPER}} .raven-categories-count',
			]
		);
	...
	protected function render_title() {
		echo '<div class="raven-categories-title-wrap">';
		if ( $this->get_instance_value( 'show_title' ) ) {
			printf(
				'<h3 class="raven-categories-title"><a href="%s">%s</a></h3>',
				esc_url( get_term_link( $this->term->term_id ) ),
				esc_html( $this->term->name )
			);
		}

		if ( $this->get_instance_value( 'show_category_count' ) ) {
			printf(
				'<div class="raven-categories-count">%s</div>',
				esc_html( $this->term->count )
			);
		}
		echo '</div>';
	}
	...
	protected function render_description() {
		...
		printf( '<p class="raven-categories-description">%s</p>', wp_kses_post( $description ) );
	}
	...
	protected function render_button() {
		...
		printf(
			...
			esc_url( get_term_link( $this->term->term_id ) ),
			wp_kses_post( $this->get_instance_value( 'button_text' ) )
		);
	}
