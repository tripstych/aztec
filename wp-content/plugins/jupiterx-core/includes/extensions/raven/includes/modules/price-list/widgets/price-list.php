...
class Price_List extends Base_Widget {
	...
	private function register_section_list() {
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .raven-price-list .raven-price-list-item .raven-price-list-header' => 'color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .raven-price-list .raven-price-list-item p.raven-price-list-description' => 'color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .raven-price-list .raven-price-list-item .raven-price-list-separator' => 'border-bottom-color: {{VALUE}};',
				],
			]
		);
	...
	private function render_image( $item, $instance ) {
		...
		return sprintf( '<img src="%s" alt="%s" />', esc_url( $image_src ), esc_attr( $item['item_title'] ) );
	}
						...
						<?php if ( ! empty( $item['item_title'] ) || ! empty( $item['item_price'] ) ) : ?>
							<div class="raven-price-list-header">
								...
								<?php if ( ! empty( $item['item_price'] ) ) : ?>
									<span class="raven-price-list-price"><?php echo wp_kses_post( $item['item_price'] ); ?></span>
								<?php endif; ?>
							</div>
						<?php endif; ?>
