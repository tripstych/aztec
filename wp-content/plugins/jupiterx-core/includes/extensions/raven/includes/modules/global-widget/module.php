<?php
namespace JupiterX_Core\Raven\Modules\Global_Widget;

defined( 'ABSPATH' ) || die();

use JupiterX_Core\Raven\Base\Module_base;
use JupiterX_Core\Raven\Modules\Global_Widget\Documents\Widget;
...
class Module extends Module_Base {
	...
	public function __construct() {
		...
		add_filter( 'elementor/editor/localize_settings', [ $this, 'add_templates_localize_data' ] );
	...
	public function is_post_type_support_elementor( $is_supported, $post_id, $post_type ) {
		...
		if ( $is_widget_template && function_exists( 'get_current_screen' ) ) {
			...
			if ( ! empty( $screen->id ) && in_array( $screen->id, [ 'elementor_library', 'edit-elementor_library' ], true ) ) {
				$is_supported = false;
			}
		}
	...
	public function remove_user_edit_cap( $allcaps, $caps, $args ) {
		$this->elementor()->modules_manager->get_modules( 'dev-tools' )->deprecation->deprecated_function( __METHOD__, '3.1.0', '$this->elementor()->documents->remove_user_edit_cap()' );

		return $this->elementor()->documents->remove_user_edit_cap( $allcaps, $caps, $args );
	}
	...
	public function set_global_widget_included_posts_list( $post_id, $editor_data ) {
		...
		$this->elementor()->db->iterate_data( $editor_data, function( $element_data ) use ( &$global_widget_ids ) {
			if ( isset( $element_data['templateID'] ) ) {
	...
	public function on_elementor_editor_init() {
		$this->elementor()->common->add_template( __DIR__ . '/views/panel-template.php' );
	}
	...
	private function get_document_data( $data, $document ) {
		...
		if ( ! ( $document instanceof Widget ) && ! empty( $data['elements'] ) ) {
			$data['elements'] = $this->elementor()->db->iterate_data( $data['elements'], function( $element ) {
