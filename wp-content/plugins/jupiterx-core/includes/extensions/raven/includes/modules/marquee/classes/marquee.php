...
abstract class Marquee extends Base_Widget {
	...
	protected function render_testimonial_content( $item ) {
		...
		if ( ! empty( $item['image']['id'] ) ) {
			...
			if ( $attachment_url ) {
				$item['image']['url'] = $attachment_url[0];
			}
		}

		$image          = ! empty( $settings['show_profile'] ) && ( isset( $item['image']['url'] ) && ! empty( $item['image']['url'] ) ) ? Group_Control_Image_Size::get_attachment_image_html( $item, 'image', 'image' ) : '';
		...
		printf(
			...
			// PHPCS - the variable $title is already escaped above.
			$rating, // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			// PHPCS - the variable $title is already escaped above.
			$twitter_handle, // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			// PHPCS - the variable $title is already escaped above.
	...
	protected function render_item_content( $item, $key ) {
		if ( 'testimonial' === $item['content_type'] ) {
			ElementorUtils::print_unescaped_internal_string( $this->render_testimonial_content( $item ) );
			return;
		}
		...
		if ( 'text' === $item['content_type'] ) {
			printf(
				'<span %1$s>%2$s</span>',
				$this->get_render_attribute_string( 'item_content_' . $key ), // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		...
		echo $frontend->get_builder_content_for_display( (int) $item['template'], true ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	}
	...
	protected function render_item( $item, $key ) {
		...
		<div <?php echo $this->get_render_attribute_string( 'item_' . esc_attr( $key ) ); ?>>
			<?php
				...
				ElementorUtils::print_unescaped_internal_string( $link_end );
			?>
		</div>
		<?php
