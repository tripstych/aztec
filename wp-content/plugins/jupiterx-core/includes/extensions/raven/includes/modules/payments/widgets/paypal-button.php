...
class Paypal_Button extends Payment_Button {
	...
	public function get_name() {
		return 'raven-paypal-button';
	}

	public static function is_active() {
		return RavenPlugin::is_active( 'paypal' );
	}

	public function get_title() {
		return esc_html__( 'PayPal Button', 'jupiterx-core' );
	}

	public function get_icon() {
		return 'raven-element-icon raven-element-paypal-button';
	}
	...
	protected function get_errors( $squash_errors = true ) {
		...
		// Other errors.
		$empty_product_price   = ( self::PAYMENT_TYPE_DONATION !== $settings['type'] && empty( floatval( $settings['product_price'] ) ) );
		$empty_donation_amount = ( self::DONATION_TYPE_FIXED === $settings['donation_type'] && empty( floatval( $settings['donation_amount'] ) ) );
	...
	protected function register_account_section() {
		...
		$this->add_control(
			'merchant_account',
			[
				'label' => esc_html__( 'Merchant Account', 'jupiterx-core' ),
				'type' => Controls_Manager::HIDDEN,
				'default' => self::API_TYPE_SIMPLE,
				'options' => [
					self::API_TYPE_SIMPLE => esc_html__( 'Default (Simple)', 'jupiterx-core' ),
					self::API_TYPE_ADVANCED => esc_html__( 'Custom (Advanced)', 'jupiterx-core' ),
				],
		...
		$this->add_control(
			'email',
			[
				'label' => esc_html__( 'PayPal Account', 'jupiterx-core' ),
				'type' => 'text',
				'dynamic' => [
					'active' => true,
				],
				'description' => esc_html__( 'Transactions made through your PayPal button will be registered under this account.', 'jupiterx-core' ),
				'label_block' => true,
				'condition' => [
		...
		$this->add_control(
			'sdk_token',
			[
				'label' => esc_html__( 'SDK Token', 'jupiterx-core' ),
				'type' => 'text',
				'dynamic' => [
					'active' => true,
				],
	...
	protected function register_sandbox_controls() {
		$this->add_control(
			'sandbox_email',
			[
				'label' => esc_html__( 'Sandbox Email Account', 'jupiterx-core' ),
				'type' => 'text',
				'dynamic' => [
					'active' => true,
				],
				'description' => esc_html__( 'This is the address given to you by PayPal when you set up a sandbox with your developer account. You can use the sandbox to test your purchase flow.', 'jupiterx-core' ),
				'label_block' => true,
				'condition' => [
					'sandbox_mode' => 'yes',
				],
