...
class Breadcrumbs extends Base_Widget {
	...
	private function register_section_breadcrumbs() {
		...
		$this->add_control(
			...
			[
				...
				'type' => 'color',
				'default' => Utils::set_old_default_value( '#a5a5a5' ),
	...
	private function raven_breadcrumbs( $html_tag = 'ol' ) {
		...
		// Open breadcrumb.
		echo '<' . ElementorUtils::validate_html_tag( $html_tag ) . ' class="breadcrumb">'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			$count = 0;
	...
	private function navtx_breadcrumbs( $html_tag ) {
		...
		if ( function_exists( 'bcn_display' ) ) {
			...
			echo '<' . $html_tag . ' class="breadcrumb" typeof="BreadcrumbList" vocab="https://schema.org/">'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
