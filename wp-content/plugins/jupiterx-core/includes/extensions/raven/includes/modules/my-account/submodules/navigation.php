...
class Navigation {
	...
	public function render_custom_navigation() {
		...
		?>
		<div class="custom-my-account-nav-<?php echo esc_attr( $settings['tabs_layout'] ); ?>">
					...
					<li class="<?php echo wc_get_account_menu_item_classes( $endpoint ); ?>">
						...
						if ( isset( $data['icon'] ) ) {
							...
							if ( false !== $icon_render_string ) {
								ElementorUtils::print_unescaped_internal_string( $icon_render_string );
							}
						}
	...
	public function get_menu_items() {
		...
		foreach ( $settings['tabs'] as $tab ) {
			...
			if ( isset( $tab['field_key'] ) ) {
				unset( $woo_tabs[ $tab['field_key'] ] );
				// This is a WooCommerce default tab.
				$default = true;
			}

			$template_id = $tab['custom_template'];
			$template    = self::get_template_by_id( $template_id, $slug );
			...
			if ( $default ) {
				// To prevent conflict with default WooCommerce tabs we add a underline at the end of URL. Otherwise this face 500 error.
				$result[ $template_id ]['template'] = $template['slug'] . '_';
			}
	...
	public static function get_template_by_id( $template_id, $slug = '' ) {
		...
		$template['slug'] = strtolower( rawurlencode( str_replace( ' ', '-', $slug ) ) );
