...
class Advanced_Accordion extends Base_Widget {
	...
	private function print_accordion_content( $item ) {
		...
		echo Plugin::instance()->frontend->get_builder_content_for_display( (int) $item['item_content_template_id'], true ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	}
					...
					<div class="jx-single-accordion-header-left">
						...
						<?php
							echo sprintf(
								...
								ElementorUtils::validate_html_tag( $title_tag ), // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	...
	private function render_icon_customized( $attributes, $icon_data ) {
		...
		if ( is_array( $icon_data['value'] ) ) {
			$icon = str_replace( '<svg', '<svg class="' . esc_attr( $attributes['class'] ) . '"', Icons_Manager::try_get_icon_html( $icon_data ) );
		}

		ElementorUtils::print_unescaped_internal_string( $icon );
	}
