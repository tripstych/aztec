...
class Social_Login extends Form {
	...
	private function register_social_login_content_section_controls() {
		...
		$this->add_control(
			'enable_twitter',
			[
				'label'        => __( 'Enable X', 'jupiterx-core' ),
				'type'         => 'switcher',
				'label_on'     => __( 'Show', 'jupiterx-core' ),
				'label_off'    => __( 'Hide', 'jupiterx-core' ),
				'return_value' => 'yes',
		...
		$this->add_control(
			...
			[
				'label'     => __( 'Label', 'jupiterx-core' ),
				'type'      => 'text',
				'default'   => __( 'Sign in with X', 'jupiterx-core' ),
				'condition' => [
					'enable_twitter' => 'yes',
				],
			]
	...
	protected function render() {
		...
		<div class="raven-social-login-wrap">
				...
				if ( 'yes' === $settings['enable_google'] ) : ?>
					<?php
						if ( Plugin::$instance->editor->is_edit_mode() ) {
							?> <script src="https://accounts.google.com/gsi/client" async defer></script><?php //phpcs:ignore
						}
					?>
					<script>
					...
					<div
						id="g_id_onload"
						data-client_id="<?php echo esc_attr( get_option( 'elementor_raven_google_client_id' ) ); ?>"
			...
			<form class="jx-raven-social-login-form">
				<input type="hidden" id="jx-raven-social-widget-post" value="<?php echo esc_attr( Utils::get_current_post_id() ); ?>">
				<input type="hidden" id="jx-raven-social-widget-form" value="<?php echo esc_attr( $this->get_id() ); ?>">
			</form>
		</div>
