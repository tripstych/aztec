...
class Form extends Base_Widget {
	...
	public function register_section_html() {
		$this->start_controls_section(
			'section_style_html',
			[
				'label' => __( 'HTML Field', 'jupiterx-core' ),
				'tab' => 'style',
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .raven-field-type-html' => 'padding-bottom: {{SIZE}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .raven-field-type-html' => 'color: {{VALUE}};',
				],
				'global' => [
		...
		$this->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} .raven-field-type-html',
				'global' => [
	...
	protected function autocomplete_address_fields() {
		...
		<script src="https://maps.googleapis.com/maps/api/js?key=<?php echo esc_js( $google_api_key ); ?>&libraries=places&callback=initRavenAddressFieldsAutocomplete" async defer></script>
		<?php
	 ...
	 * @since 4.3.0
	 */
	public function add_popup_to_location( $settings ) {
		if ( ! isset( $settings['popup_action'] ) ) {
			return;
		}

		if ( 'open' !== $settings['popup_action'] || empty( $settings['popup_action_popup_id'] ) ) {
			return;
		}
