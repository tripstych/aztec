...
class SoundCloud extends Base {
	public static function render_item( $data, $settings ) {
		...
		$meta_data                   = self::get_meta_data( $data, 'soundcloud_poster' );
		...
		<div class="gallery-item"
			...
			<?php endif; ?>
		>
			<div class="type-audio soundcloud">
				<?php Utils::print_unescaped_internal_string( self::poster_image( $data, $settings ) ); ?>
			</div>
	...
	private static function get_embed_data( $data ) {
		$encoded_url    = rawurlencode( $data['soundcloud_url']['url'] );
		$embed_url      = 'https://soundcloud.com/oembed?url=' . $encoded_url;
	...
	private static function render_lightbox_html( $data ) {
		...
		ob_start();
		?>
		<div class="raven-media-gallery-lightbox-item type-audio soundcloud">
			<div class="iframe-container">
				<?php
				if ( ! empty( $embed_data ) ) {
					Utils::print_unescaped_internal_string( $embed_data->html );
				} else {
	...
	private static function poster_image( $data, $settings ) {
		// WPML compatibility.
		$data['soundcloud_poster']['id']  = apply_filters( 'wpml_object_id', $data['soundcloud_poster']['id'], 'attachment', true );
		$data['soundcloud_poster']['alt'] = get_post_meta( $data['soundcloud_poster']['id'], '_wp_attachment_image_alt', true );

		$lazy       = self::is_lazy_load( $settings ) ? 'loading="lazy"' : '';
		$poster_url = Group_Control_Image_Size::get_attachment_image_src( $data['soundcloud_poster']['id'], 'thumbnail_image', $settings );
		$play_icon  = self::render_play_icon( $settings );
		$zoom_img   = '';

		if ( 'zoom' === $settings['image_hover_animation'] && ! empty( $data['soundcloud_poster']['id'] ) ) {
			$full_poster = wp_get_attachment_image_url( $data['soundcloud_poster']['id'], 'full' );
		...
		return sprintf(
			...
			$zoom_img,
			esc_url( $poster_url ),
			! empty( $data['soundcloud_poster']['alt'] ) ? esc_html( $data['soundcloud_poster']['alt'] ) : '',
			$lazy
		);
	}
}
