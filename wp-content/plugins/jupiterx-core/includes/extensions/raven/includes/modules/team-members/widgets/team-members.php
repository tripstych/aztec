...
class Team_Members extends Base_Widget {
	...
	private function register_style_name_section() {
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .raven-team-members-wrapper .team-member--name' => 'color: {{VALUE}}',
					'{{WRAPPER}} .raven-team-members-wrapper .team-member--name a' => 'color: {{VALUE}}',
				],
	...
	public function render_team_member_card( $settings, $member ) {
		...
		<div class="team-member--image-wrapper">
			...
			<img
				...
				src="<?php echo esc_url( $this->get_profile_picture_url( $settings, $member ) ); ?>"
				alt="<?php echo esc_attr( $this->get_profile_picture_alt( $member ) ); ?>"
			>
			<?php
		...
		Utils::print_unescaped_internal_string( $this->get_member_content( $settings, $member ) );
	...
	private function get_member_content( $settings, $member ) {
		...
		<div class="team-member--content">
			...
			Utils::print_unescaped_internal_string( $this->get_member_name( $settings, $member ) );
			Utils::print_unescaped_internal_string( $this->get_position( $member ) );
			Utils::print_unescaped_internal_string( $this->get_description( $member ) );
	...
	private function get_member_name( $settings, $member ) {
		...
		if ( 'title' === $member['link_type'] ) {
			$html = sprintf(
				...
				esc_html( $tag ),
				$this->get_member_link_attributes( $member ),
				esc_html( $member['name'] )
			);
		}
	...
	private function get_profile_picture_url( $settings, $member ) {
		...
		// WPML compatibility.
		$member['profile_picture']['id'] = apply_filters( 'wpml_object_id', $member['profile_picture']['id'], 'attachment', true );
