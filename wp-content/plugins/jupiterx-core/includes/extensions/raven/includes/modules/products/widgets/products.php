...
class Products extends Base_Widget {
	...
	private function register_section_add_to_cart() {
		...
		if ( in_array( get_option( 'elementor_experiment-e_font_icon_svg', 'default' ), [ 'active', 'default' ], true ) ) {
			$this->add_responsive_control(
	...
	protected function render() {
		...
		<div
			...
			<?php $this->archive_query_parameters(); ?>
		>
			<?php
				ElementorUtils::print_unescaped_internal_string( $this->get_widget_title( $settings, $query, $query_results ) );
	...
	private function get_widget_title( $settings, $query, $query_results ) {
		...
		return '<h2 class="raven-wc-products-title">' . wp_kses_post( $text ) . '</h2>';
	}
	...
	private function archive_query_parameters() {
		...
		printf( 'data-raven-archive-query="%s"', esc_attr( $json_query ) );
	}
}
